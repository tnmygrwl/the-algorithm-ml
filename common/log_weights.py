...
def weights_to_log(
  ...
  logging.info(
      f"Using dmp parameters: {[name for name, _ in model._dmp_wrapped_module.named_parameters()]}"
  )
  for param_name, params in named_parameters:
    how = how_to_log if callable(how_to_log) else how_to_log.get(param_name)
    if not how:
